...
for sub_idx, sub_name in enumerate(sub_names):
    ...
    if not os.path.exists(label_file):
        ...
        df_img_name = []
        # find imagenet task, only use the first four sessions
        imagenet_sess = [_ for _ in os.listdir(sub_events_path) if ('imagenet' in _) and int(_[-2:]) < 5]
    ...
    if not os.path.exists(beta_sub_path):
        ...
        for i_ses in range(num_ses):
            for i_run in range(num_run):
                ...
                beta_run_path = pjoin(ciftify_path, sub_name, 'results', run_name, f'{run_name}_beta.dscalar.nii')
                beta_sub[i_ses, i_run*num_trial : (i_run + 1)*num_trial, :] = np.asarray(nib.load(beta_run_path).get_fdata())
...
save_ciftifile(save_relibility, pjoin(result_path, f'imagenet_{clean_code}_cross-ses-reliability.dtseries.nii'))
print('##### save #####')
